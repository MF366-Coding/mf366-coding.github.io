...
1. *Generic Case:* =ECHO= sucessfully writes to a file.
2. *~FileExistsError~:* =ECHO= fails and throws an Exception. This happens if the file already exists.
3. *Reverts to =COUT=:* Yes, in theory, you /can/ use =ECHO= instead of =COUT=, if you give =ECHO= an empty string instead of an actual filepath. However, it's not really that good of an idea.

| Summary                                                                      | Arguments                                                                                                                             | Returns | Exceptions                        | Example                                 |
|------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------+---------+-----------------------------------+-----------------------------------------|
| Writes to a file or reverts back to =COUT= if the filepath is an empty string. | ~DataToWrite~ - string, the data to write; ~FilePath~ - string, the filepath to write to (if empty, reverts back to =COUT ?? /DataToWrite/;=) | None    | ~FileExistsError~ - the file exists | =ECHO ?? "Hello, World!" ?? "hello.txt";= |
...
| Summary                                                                      | Arguments                                                                                                                             | Returns | Exceptions | Example                                 |
|------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------+---------+------------+-----------------------------------------|
| Writes to a file or reverts back to =COUT= if the filepath is an empty string. | ~DataToWrite~ - string, the data to write; ~FilePath~ - string, the filepath to write to (if empty, reverts back to =COUT ?? /DataToWrite/;=) | None    | None       | =ECHO ?? "Hello, World!" ?? "hello.txt";= |
...
The syntax for cache grabs is the following and can be sed as arguments for keywords:
#+begin_src
AB:CD:E:F:G
#+end_src
...
- *B:* an integer. This is used as the start index for the cache grab. For example, ~0~ in this case means *start from the beggining*.
...
- *D:* a single character, must be one of ~l~ or ~e~. This is the value that decides whther value *C* is interpreed as the *end index* (~e~) or the *length of the cache grab* (~l~).
- *E:* an integer. Represents the *step value*. This represets the pattern to perform. Let's say we're cache grabbing ~Hello, World!~. Using a *step value* of 1 (default), we'd get ~Hello, World!~ but a *step value of 2* would give us the same string, but skipping each odd-indexed character (indexing starts at 0). So, ~Hlo ol!~.
...
| *F* Value (DataType Conversion) | Meaning                                                                                                                     |
|-------------------------------+-----------------------------------------------------------------------------------------------------------------------------|
|                             0 | *Void* - uncompatible, will throw an error.                                                                                   |
...
- *User Feedback.* Hey, the user knows best, right? You can obtain filepaths, make decisions and much more, based on the user input.
