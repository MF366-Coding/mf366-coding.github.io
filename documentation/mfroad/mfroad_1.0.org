...
A code path can be defined rather easily. Just keep in mind MFRoad files are parsed from the beggining, so the valid conditions at the start will porevent the ones at the end fron running.
...
* ~any~
Sometimes, the architecture may not matter. Both ~<system>~ and ~<architecture>~ fields can be repalced by ~any~, which means any system/architecture will be a match.
#+begin_src cxsetup
...
|-----------+---------------------------------------------------+-------------------------------------------------------------------------------|
| =???=       | Returns a result - the valid code path to follow. | =darwin:any???darwin.cs; // darwin is MacOS;=                                   |
...
- *Parse for your favorite programming language.* Python is good, but you proabbly need a parser for a different programming language.

* Not in the mood for MFRoad?
I understand if you ddn't like the language, and that's why I offer an alternative: *JSON*.
